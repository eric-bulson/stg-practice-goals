...
public class PracticeReportDao {
	...
	public GoalsReport getGoalsReport() {
		GoalsReport result = namedJdbcTemplate.execute(
				"select "
				+ "tr.type, "
				+ "tt.type typeName, "
				+ "tr.practice, "
					...
					public GoalsReport doInPreparedStatement(PreparedStatement ps)
						...
						while(rs.next()){
							...
							if (goalsMap.containsKey(practiceName)){
								goalsMap.get(practiceName).put(rs.getString("typeName"), new ProgressReport(rs.getLong("actual"), rs.getLong("expected"), rs.getLong("totalTimeInSeconds")));
							}else{
								HashMap<String, ProgressReport> newMap = new HashMap<>();
								newMap.put(rs.getString("typeName"), new ProgressReport(rs.getLong("actual"), rs.getLong("expected"), rs.getLong("totalTimeInSeconds")));
